<?php
// ── 404 GUARD: verify the album exists before any HTML is emitted, so a
//    missing/empty album returns a real 404 instead of a thin "soft 404" page. ──
require_once 'admin/db.php';
$albumName = isset($_GET['album']) ? htmlspecialchars($_GET['album']) : '';
$albumExists = false;
if (!empty($albumName)) {
    $stmt = $conn->prepare("SELECT COUNT(*) AS c FROM project_images WHERE album = ?");
    $stmt->bind_param("s", $albumName);
    $stmt->execute();
    $res = $stmt->get_result();
    $albumExists = ($res && ($row = $res->fetch_assoc()) && (int)$row['c'] > 0);
    $stmt->close();
}
if (!$albumExists) {
...
<head>
    ...
    <title>Album Not Found – Greenwood Philippines</title>
    <style>
...
<head>
    ...
    $seoAlbumUrl   = 'https://greenwoodphilippines.com/view_album.php' . (!empty($albumName) ? '?album=' . urlencode($albumName) : '');
    $seoAlbumImage = !empty($headerImagePath)
        ? 'https://greenwoodphilippines.com/' . ltrim($headerImagePath, '/')
        : 'https://greenwoodphilippines.com/assets/images/project_cover.webp';
    ?>
